...
- [Quick Start](airl2/quick_start.md)
- [Pre-Estimation Checks](airl2/pre_estimation.md)
- [Simulation Study](airl2/validation.md)
- [Serialized-Content Example](airl2/serialized_content.md)

AIRL2 extends adversarial inverse reinforcement learning to populations with
...
- **Absorbing-state anchor.** Every action has zero flow reward at the
  absorbing terminal state, $g_k(s_{\text{absorb}}, a) = 0$, and the shaping
  potential satisfies $h_k(s_{\text{absorb}}) = 0$ for all $k$. Together with
  the exit-action anchor this pins the value potential, so that the recovered
  shaping function equals the true value function $V_k^*$.
...
Analytic standard errors for adversarial reward parameters are not available.
With `compute_se=True`, the public wrapper resamples individuals, refits from
cold starts, and aligns every draw to the fitted segment labels. It forms
normal intervals for segment reward cells, policies, and prior shares using a
fixed 4.0 pilot multiplier. Segment-prior standard errors also have a binomial
floor. `se_` remains `None` because these are functional intervals, not a
Hessian for the adversarial parameters.

## Algorithm

```text
Algorithm  AIRL2 (EM adversarial IRL with anchor identification)
...
1   initialize g_k, h_k, lambda_k, and q_ik with the configured scheme
2   enforce g_k(s, exit_action) := 0 for all s, k
3   enforce g_k(absorbing_state, a) := 0 and h_k(absorbing_state) := 0
...
18              enforce both reward anchors and h_k(absorbing_state) := 0
19              re-solve soft Bellman for pi_k via hybrid value iteration
20          until policy change < airl_convergence_tol or max_airl_rounds is reached
21      mixture_ll := sum_i log sum_k lambda_k prod_t pi_k(a_it | s_it)
22  until relative mixture-LL change < em_tol or max_em_iterations is reached
...
at the cost of slower convergence. The public wrapper defaults to
`reward_type="linear"`, which projects onto a supplied feature matrix. It
defaults to `initialization="behavioral_anchor"`, which clusters trajectories
by observed action shares and inverts the anchored soft policy into a starting
reward. Set `initialization="random"` to use random initialization.

## System View

AIRL2 adds a segment layer to anchored adversarial reward recovery. Each
...
AIRL2 sits at the intersection of adversarial IRL and structural dynamic
...
estimators (NFXP, CCP, MPEC, UFXP), AIRL2 does not maximize a likelihood
and does not report asymptotic parameter standard errors. It provides a
cluster bootstrap for segment reward, policy, and prior functionals. AIRL2 is
appropriate when segment-level reward recovery and counterfactual behavior are
the research target rather than inference on likelihood parameters.
...
print(model.segment_policies_.shape)
```
...
exit_action = model.exit_action
for k, rw in enumerate(seg_rewards):
    rw_cf = jnp.array(rw).at[:, 0].add(0.5)
    rw_cf = rw_cf.at[:, exit_action].set(0.0)
    rw_cf = rw_cf.at[model.absorbing_state, :].set(0.0)  # re-enforce both anchors
...
AIRL2 is evaluated on a synthetic serialized-content environment with two
...
In this cell, the transition-change counterfactual produces more regret than
the reward shift or action removal. The recovered reward must transfer across
a dynamically different world without a likelihood-based correction.

AIRL2 is not included in the current cross-estimator simulation studies,
...
- Estimator source: [`econirl.estimation.adversarial.airl2`](https://github.com/rawatpranjal/EconIRL/blob/main/src/econirl/estimation/adversarial/airl2.py).
- Validation runner: [`validation/estimators/airl2/run.py`](https://github.com/rawatpranjal/EconIRL/blob/main/validation/estimators/airl2/run.py).
- Results file: [`airl2.json`](https://github.com/rawatpranjal/EconIRL/blob/main/validation/results/airl2.json).
...
- [Pre-Estimation Checks](airl2/pre_estimation.md)
- [Simulation Study](airl2/validation.md)
- [Counterfactuals](airl2/counterfactuals.md)
- [Serialized-Content Example](airl2/serialized_content.md)
...
airl2/quick_start
airl2/pre_estimation
airl2/validation
airl2/counterfactuals
airl2/serialized_content
```
