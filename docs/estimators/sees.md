...
The recovery study resimulates and refits across 50 replications. The estimator recovers all 32 value-function weights: the aggregate recovery RMSE is 0.0022, and the true value falls inside the 95% interval for all 32 of 32 weights. The figure shows the per-weight recovery-error distribution.
