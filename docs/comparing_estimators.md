...
establish reward recovery. Recovery also needs a normalization, enough support,
...
| [GLADIUS](estimators/gladius.md) | You want paper-reference neural Q and continuation modeling with anchored reward recovery. | Dynamic discrete-choice trajectories; credible anchor rewards; transitions supplied only for post-fit planning. | Anchored neural reward recovered from Q and continuation objects. | High-dimensional encoded state features. | No credible anchor exists, or you need tabular likelihood estimation. | Recovery checks, bootstrap calibration, and a partial Table 2 match (five of six sample sizes). |
...
| GLADIUS paper minimax | Anchored neural reward and action contrasts. | Conditional. | Credible anchor, learned Q and continuation objects, action-contrast rank, support, and planning transitions for counterfactuals. |
