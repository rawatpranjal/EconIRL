"""Tests for NFXP wrapper refactoring: RewardSpec, TrajectoryPanel, EstimatorProtocol.

Tests that the NFXP sklearn wrapper:
...
6. All existing methods (simulate, counterfactual, predict_proba, summary) still work
"""
...
def fitted_model(bus_df):
    """Fitted NFXP model shared across test classes."""
    model = NFXP(n_states=_N_STATES, discount=_DISCOUNT, verbose=False)
...
class TestBackwardCompat:
    """NFXP still works with the original DataFrame API."""
...
class TestRewardSpec:
    """NFXP accepts a RewardSpec for custom features."""
    ...
    def test_fit_with_reward_spec_argument(self, bus_df):
        ...
        model = NFXP(n_states=n, discount=_DISCOUNT, verbose=False)
        model.fit(
            bus_df,
            state="mileage_bin",
            action="replaced",
    ...
    def test_fit_with_reward_spec_in_constructor(self, bus_df):
        ...
        model = NFXP(n_states=n, discount=_DISCOUNT, utility=spec, verbose=False)
        model.fit(bus_df, state="mileage_bin", action="replaced", id="bus_id")
        assert model.params_ is not None
        assert model.reward_spec_ is spec
...
class TestTrajectoryPanelInput:
    """NFXP accepts TrajectoryPanel as data input."""
    ...
    def test_dataframe_without_column_names_raises(self, bus_df):
        model = NFXP(n_states=_N_STATES, verbose=False)
...
class TestNewAttributes:
    """Fitted NFXP has policy_, value_, pvalues_."""
...
class TestProtocol:
    """NFXP satisfies the EstimatorProtocol."""
...
class TestExistingMethodsStillWork:
    ...
    def test_repr_unfitted(self):
        r = repr(NFXP(n_states=_N_STATES))
