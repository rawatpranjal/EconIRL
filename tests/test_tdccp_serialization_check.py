"""Fresh-process serialization checks for the TD-CCP workflow."""
...
def test_serialization_runner_uses_a_fresh_process(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "validation/estimators/tdccp/serialization_check.py",
            "--output",
            str(output),
        ],
        check=False,
...
def test_release_receipt_records_wheel_process_parity() -> None:
    payload = json.loads(
        Path("validation/results/tdccp_serialization.json").read_text(encoding="utf-8")
    )
    assert payload["status"] == "ready"
    assert payload["fresh_process"] is True
    assert payload["summary_equal"] is True
