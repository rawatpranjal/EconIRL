"""Pure aggregation and receipt tests for CCP bootstrap calibration."""
...
from validation.estimators.ccp.bootstrap_calibration import (
    calibration_gates,
    summarize_bootstrap,
)
...
def test_release_artifact_records_full_trajectory_calibration() -> None:
    payload = json.loads(
        Path("validation/results/ccp_bootstrap_calibration.json").read_text(encoding="utf-8")
    )
    ...
    assert payload["design"]["transition_policy"] == ("supplied transition tensor held fixed")
