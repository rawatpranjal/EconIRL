...
def main():
    ...
    df_lin = panel_lin.to_dataframe().rename(columns={"state": "mileage_bin", "action": "replaced"})
    print(
        f"Linear DGP: {len(df_lin):,} obs, {N_BUSES} buses, "
    ...
    print(
        f"Nonlinear DGP: {len(df_nl):,} obs, {N_BUSES} buses, "
    ...
    print(
        f"NFXP (linear):     theta_c={nfxp_lin.params_['theta_c']:.6f}, "
    ...
    print(
        f"NFXP (nonlinear):  theta_c={nfxp_nl.params_['theta_c']:.6f}, "
    ...
    ax1.set_xlabel("Mileage bin")
    ax1.set_ylabel("P(replace)")
    ...
    ax1.plot(
        np.asarray(deltas), sw["g_lin"]["mean_action_prob"], "s-", color=ORANGE, label="GLADIUS"
    )
    ax1.set_xlabel("Replacement penalty")
    ...
    ax2.plot(
        np.asarray(deltas),
        sw["nfxp_nl"]["mean_action_prob"],
        "o-",
        color=BLUE,
        label="NFXP (misspec)",
    )
    ax2.plot(np.asarray(deltas), sw["g_nl"]["mean_action_prob"], "^-", color=GREEN, label="GLADIUS")
    ax2.set_xlabel("Replacement penalty")
    ax2.set_ylabel("Mean P(replace)")
    ax2.legend()
    fig.tight_layout()
    ...
    for ax, pairs, title in [
        ...
        for label, color, name in pairs:
            ax.plot(
                states,
                np.asarray(t_cfs[label].baseline_policy[:, 1]),
                color=color,
                lw=1.5,
                label=f"{name} base",
            )
            ax.plot(
                states,
                np.asarray(t_cfs[label].counterfactual_policy[:, 1]),
                "--",
                color=color,
                lw=1.5,
                label=f"{name} CF",
    ...
    for ax, (label, name) in zip(
        axes.flat,
        [
            ("nfxp_lin", "NFXP (linear)"),
            ("g_lin", "GLADIUS (linear)"),
            ("nfxp_nl", "NFXP (nonlinear, misspec)"),
            ("g_nl", "GLADIUS (nonlinear)"),
        ],
    ):
        res = cs_cfs[label]
        ...
        ax.set_title(name)
        ax.legend(fontsize=8)
    axes[0, 0].set_ylabel("P(replace)")
    axes[1, 0].set_ylabel("P(replace)")
    ...
    print(
        f"  {'NFXP (linear)':>20s}  {nfxp_lin.params_['theta_c']:10.6f}  "
        f"{nfxp_lin.params_['RC']:8.4f}  {'—':>6s}"
    )
    print(
        f"  {'GLADIUS (linear)':>20s}  {sieves['g_lin']['theta'][0]:10.6f}  "
        f"{abs(sieves['g_lin']['theta'][1]):8.4f}  {sieves['g_lin']['r_squared']:6.4f}"
    )
    print(
        f"  {'GLADIUS (nonlinear)':>20s}  {sieves['g_nl']['theta'][0]:10.6f}  "
    ...
    for ax, r, sieve, title in [
        (ax1, g_lin_r, sieves["g_lin"], f"GLADIUS linear (R²={sieves['g_lin']['r_squared']:.2f})"),
        (ax2, g_nl_r, sieves["g_nl"], f"GLADIUS nonlinear (R²={sieves['g_nl']['r_squared']:.2f})"),
    ]:
