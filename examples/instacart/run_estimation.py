"""Instacart Grocery Reorder: Structural estimation with post-estimation diagnostics.

Demonstrates NFXP, CCP, and MCE-IRL on a grocery reorder DDC problem modeled
on Instacart data. Known ground truth enables parameter recovery evaluation.
...
def main():
    print("=" * 65)
    print("Instacart Grocery Reorder (20 states, 2 actions)")
    print("=" * 65)

    env = InstacartEnvironment(discount_factor=0.95)
    panel = load_instacart(n_individuals=2000, n_periods=52, as_panel=True, seed=42)
    ...
    print(f"  Train: {train.num_individuals} consumers, {train.num_observations} obs")
    print(f"  Test:  {test.num_individuals} consumers, {test.num_observations} obs")
    ...
    # Counterfactual: free delivery
    print("\n" + "=" * 65)
    print("Counterfactual: Free Delivery (reorder_cost = 0)")
    print("=" * 65)
    best = results["NFXP"]
    reorder_cost_idx = env.parameter_names.index("reorder_cost")
    new_params = best.parameters.at[reorder_cost_idx].set(0.0)
    cf = counterfactual_policy(best, new_params, utility, problem, transitions)
    print(f"Welfare change: {float(cf.welfare_change):+.3f}")

    # Elasticity
    print("\n--- Reorder Cost Elasticity ---")
    ea = elasticity_analysis(
        best, utility, problem, transitions,
        parameter_name="reorder_cost",
