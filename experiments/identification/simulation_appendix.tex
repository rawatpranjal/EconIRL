...
GLADIUS parameterizes both $Q(s,a)$ and $\hat{\mathbb{E}}[V(s') \mid s,a]$ with neural networks, trained jointly via logit NLL plus a Bellman consistency penalty on observed $(s,a,s')$ transitions. It extracts reward via the same inverse Bellman operator as IQ-Learn. The MLP architecture shares parameters across states, providing implicit regularization over tabular $Q$.
...
The tabular $\chi^2$ IQ-Learn objective has multiple local optima in finite samples. We compare three finite-sample estimators. The reduced-form logit converges to $A^*$, which is the wrong target for Type II counterfactuals. The anchor-constrained MLE adds soft penalties on the implied reward at exit and absorbing states to the logit NLL. GLADIUS uses neural $Q$ and EV networks.
