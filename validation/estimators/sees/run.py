...
Usage:
    cd /path/to/econirl
    PYTHONPATH=src:. python validation/estimators/sees/run.py
"""
...
ROOT = HERE.parents[2]
JSON_OUT = ROOT / "validation" / "results" / "sees.json"
...
def render_results_tex(records: list[dict[str, Any]]) -> str:
    ...
    add(
        ...
        "a sanity check for the historical index-based sieve; the "
        "high-dimensional action-dependent DGP is the main SEES validation "
        "because it uses the encoded-state sieve."
    )
    add("")
    ...
    if high_record is not None:
        high_summary = high_record["payload"]["summary"]
        add(
            "The primary high-dimensional cell ran in "
            f"{fmt(high_summary.estimation_time, 2)} seconds."
        )
    add("")
...
def compact_payload(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    release_status = (
        "certified" if primary_result.get("cell_id") == PRIMARY_CELL_ID and all_gates_pass
        else "diagnostic rerun"
    )
    return {
        "estimator": ESTIMATOR,
        "paper_target": (
            "Sieve value approximation for structural dynamic discrete choice, "
            "validated on the encoded-state known-truth DGP."
        ),
...
def finite_jsonable(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {str(k): finite_jsonable(v) for k, v in value.items()}
