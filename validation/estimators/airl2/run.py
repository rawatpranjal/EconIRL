#!/usr/bin/env python3
"""Generate AIRL2 known-truth validation artifacts.

AIRL2 is evaluated on an anchored two-segment serialized-content known-truth
...
ROOT = HERE.parents[2]
JSON_OUT = ROOT / "validation" / "results" / "airl2.json"
DEFAULT_OUTPUT_DIR = Path("/tmp/econirl_airl2_primer_known_truth")
PRIMARY_CELL_ID = "airl2_paper_identification"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "anchored AIRL2 paper-style validation cell."
        ),
    )
    ...
    print("AIRL2 primer: running known-truth validation")
...
def run_validation_cell(cell_id: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_dir = args.output_dir / f"{cell.cell_id}_airl2_primer_{run_hash}"
...
def render_results_tex(records: list[dict[str, Any]]) -> str:
    ...
    add("% Auto-generated by airl2_run.py. Do not edit by hand.")
    add("% Known-truth DGP cells: " + cell_list)
    add("")
    add(r"\subsection{Known-Truth Validation Results}")
    add(
        "The AIRL2 validation target is a serialized-content DGP with two "
    ...
    add(r"\caption{AIRL2 known-truth validation cells.}")
    ...
    add(r"\begin{table}[H]")
    add(r"\centering\small")
    add(r"\caption{AIRL2 segment recovery metrics. Lower is better except assignment.}")
    add(r"\resizebox{\textwidth}{!}{%")
...
def compact_payload(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "status": status,
        "interpretation": (
            "AIRL2 is a diagnostic failure on the anchored serialized-content "
            "latent-segment known-truth cell."
            if primary_failed
            else (
                "AIRL2 passes the anchored serialized-content latent-segment "
                "known-truth cell."
            )
        ),
