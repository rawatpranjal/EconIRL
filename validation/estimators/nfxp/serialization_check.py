#!/usr/bin/env python3
"""Verify NFXP pickle parity in a fresh Python process."""
...
DEFAULT_OUTPUT = ROOT / "validation" / "results" / "nfxp_serialization.json"


def build_workflow() -> tuple[ArrayMDP, np.ndarray, RewardSpec]:
    """Build a compact identified replacement problem."""
    n_states = 12
    transitions: np.ndarray = np.zeros((2, n_states, n_states), dtype=np.float64)
    ...
    features: np.ndarray = np.zeros((n_states, 2, 2), dtype=np.float64)
    features[:, 0, 0] = -condition
    features[:, 1, 1] = -1.0
    reward = RewardSpec(features, names=["condition_cost", "replacement_cost"])
    env = ArrayMDP(
        transitions,
        features,
        ...
        scale_parameter=1.0,
        parameter_names=["condition_cost", "replacement_cost"],
        seed=52_001,
    )
    return env, transitions, reward


def fit_model() -> NFXP:
    """Fit the public estimator on a deterministic synthetic panel."""
    env, transitions, reward = build_workflow()
    panel = simulate_panel(env, n_individuals=80, n_periods=25, seed=52_001)
    model = NFXP(
...
def snapshot(model: NFXP) -> dict[str, Any]:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="econirl-nfxp-pickle-") as temp_name:
        temp = Path(temp_name)
        model_path = temp / "nfxp.pkl"
