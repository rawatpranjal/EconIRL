#!/usr/bin/env python3
"""Monte-Carlo parameter-recovery driver for the UFXP estimator.
...
each replication, re-fits UFXP, and collects the sampling distribution of the
4 recovered structural parameters. Produces a caterpillar figure and a JSON.

Usage:
    cd /path/to/econirl
    PYTHONPATH=src:. python validation/estimators/ufxp/recovery_mc.py --n-reps 200

    # fast smoke test
    PYTHONPATH=src:. python validation/estimators/ufxp/recovery_mc.py --n-reps 3
"""
...
ESTIMATOR = "UFXP"
JSON_OUT = ROOT / "validation" / "results" / "ufxp_recovery.json"
FIG_OUT = ROOT / "docs" / "_static" / "estimators" / "ufxp_recovery.png"
