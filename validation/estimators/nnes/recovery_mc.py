#!/usr/bin/env python3
"""Monte-Carlo parameter-recovery driver for the NNES estimator.
...
each replication, re-fits NNES, and collects the sampling distribution across
all 32 value-function-weight params. Stores full per-param stats in the JSON
(reproducibility) and renders a compact error-distribution histogram (a
32-row caterpillar would be too crowded for a public page).
...
Usage:
    cd /path/to/econirl
    PYTHONPATH=src:. python validation/estimators/nnes/recovery_mc.py --n-reps 200

    # fast smoke test
    PYTHONPATH=src:. python validation/estimators/nnes/recovery_mc.py --n-reps 3
"""
...
ESTIMATOR = "NNES"
JSON_OUT = ROOT / "validation" / "results" / "nnes_recovery.json"
FIG_OUT = ROOT / "docs" / "_static" / "estimators" / "nnes_recovery.png"
