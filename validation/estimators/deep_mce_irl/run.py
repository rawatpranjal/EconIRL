...
ROOT = HERE.parents[2]
JSON_OUT = ROOT / "validation" / "results" / "deep_mce_irl.json"
...
def render_results_tex(records: list[dict[str, Any]]) -> str:
    ...
    add(
        "The primary validation target is an anchored raw nonlinear reward map "
        "learned from demonstrations under known stochastic transitions and "
    ...
    for record in records:
        ...
        add(
            f"The {tex_text(validation_display_name(cell_id))} passes "
            f"{passed}/{len(gates)} "
            "hard gates."
        )
        if failed:
