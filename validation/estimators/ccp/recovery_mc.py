#!/usr/bin/env python3
"""Monte-Carlo parameter-recovery driver for the CCP estimator.
...
each replication, re-fits CCP, and collects the sampling distribution of the
4 recovered structural parameters. Produces a caterpillar figure and a JSON.

Usage:
    cd /path/to/econirl
    PYTHONPATH=src:. python validation/estimators/ccp/recovery_mc.py --n-reps 200

    # fast smoke test
    PYTHONPATH=src:. python validation/estimators/ccp/recovery_mc.py --n-reps 3
"""
...
ESTIMATOR = "CCP"
JSON_OUT = ROOT / "validation" / "results" / "ccp_recovery.json"
FIG_OUT = ROOT / "docs" / "_static" / "estimators" / "ccp_recovery.png"
