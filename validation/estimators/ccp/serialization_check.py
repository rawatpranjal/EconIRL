#!/usr/bin/env python3
"""Verify CCP pickle parity in a fresh Python process."""
...
DEFAULT_OUTPUT = ROOT / "validation" / "results" / "ccp_serialization.json"
...
def fit_model() -> CCP:
    ...
    panel = simulate_panel(env, n_individuals=80, n_periods=25, seed=72_001)
    model = CCP(
...
def snapshot(model: CCP) -> dict[str, Any]:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="econirl-ccp-pickle-") as temp_name:
        temp = Path(temp_name)
        model_path = temp / "ccp.pkl"
