...
ROOT = HERE.parents[2]
JSON_OUT = ROOT / "validation" / "results" / "ccp.json"
DEFAULT_OUTPUT_DIR = Path("/tmp/econirl_ccp_primer_known_truth")
CELL_ID = "canonical_low_action"
ESTIMATOR = "CCP"
PAPER_TARGET = "Hotz-Miller and NPL pseudo-likelihood for tabular DDC"
RELEASE_STATUS = "Certified with support conditions"
...
def main() -> None:
    ...
    JSON_OUT.write_text(
        json.dumps(compact_payload(payload), indent=2, sort_keys=True, allow_nan=False)
        + "\n",
        encoding="utf-8",
    )
...
def compact_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "result": _finite_jsonable(payload),
    }
...
def render_results_tex(payload: dict[str, Any], dgp: Any) -> str:
    ...
    add(validation_context([payload["cell"].cell_id]))
    add("")
