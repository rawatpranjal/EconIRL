"""TT2 transfer test for GLADIUS. The recovered reward should generalize
because GLADIUS does not require a transition matrix at fit time.

Reproduces: row TT2 of transfer_results.csv.
"""
...
est = GLADIUSEstimator(config=GLADIUSConfig(q_hidden_dim=64, v_hidden_dim=64, max_epochs=200))
res = est.estimate(panel=panel, utility=linear, problem=problem, transitions=P_train)

# GLADIUS recovers an implied reward from Q - beta * V[s'] averaged over observed transitions.
r_implied = jnp.asarray(getattr(res, "recovered_reward", linear.compute(jnp.array([0.001, 3.0]))), dtype=jnp.float64)

op_test = SoftBellmanOperator(problem, P_test)
val_test = policy_iteration(op_test, r_implied, tol=1e-10, max_iter=200, eval_method="matrix")
...
row = {"estimator": "GLADIUS", "percent_transfer_value": f"{pct:.2f}"}
