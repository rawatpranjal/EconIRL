...
Listing~\ref{lst:nfxp} shows the user-facing session that loads the bundled dataset, fits the nested fixed point estimator, and prints the regression-style summary.

\begin{lstlisting}[style=econirlsession,caption={Fitting the nested fixed point estimator on the Rust bus panel. Reproduced by \code{code\_snippets/listing2\_nfxp\_example.py}.},label={lst:nfxp}]
...
>>> result = NFXP().estimate(panel=panel,
...
Log-Likelihood: -1900.33    Hessian Condition: 7.3e4
\end{lstlisting}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{fig1_rust_bus_ccp}
\caption{Estimated replacement probability under the baseline parameters and under a counterfactual that reduces the replacement cost by 25 percent. Reproduced by \code{code\_snippets/fig1\_rust\_bus\_ccp.py} with seed 42 on Apple M2 Pro CPU, \pkg{econirl} 0.0.4.}
\label{fig:rust_ccp}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{fig2_rust_bus_value}
\caption{Converged value function $V(s)$ over the discretized mileage state under the baseline estimates. Reproduced by \code{code\_snippets/fig2\_rust\_bus\_value.py} with seed 42 on Apple M2 Pro CPU, \pkg{econirl} 0.0.4.}
...
\begin{lstlisting}[style=econirlsession,caption={Fitting MCE-IRL on the same Rust bus panel. Reproduced by \code{code\_snippets/listing3\_mce\_irl\_example.py}.},label={lst:mce}]
...
>>> reward = ActionDependentReward(feature_matrix=linear.feature_matrix,
...     parameter_names=linear.parameter_names)
>>> result_irl = MCEIRL(config=MCEIRLConfig(optimizer="L-BFGS-B",
...     inner_solver="policy", outer_max_iter=500)).estimate(
...     panel=panel, utility=reward, problem=env.problem_spec,
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{fig3_mce_irl_reward}
\caption{Recovered reward from MCE-IRL against the structural utility from NFXP on the same Rust bus panel. The two curves coincide up to a constant. Reproduced by \code{code\_snippets/fig3\_mce\_irl\_reward.py} with seed 42 on Apple M2 Pro CPU, \pkg{econirl} 0.0.4.}
...
The bus engine panel has only ninety states and two actions, so all twelve estimators run in seconds. The Keane-Wolpin occupational choice panel of \citet{keane1997} is more demanding. Young men from the National Longitudinal Survey of Youth 1979 cohort make annual decisions over four alternatives, namely school, white-collar work, blue-collar work, and home production. The state space comprises completed years of schooling, occupation-specific experience, and age, which together generate over five thousand reachable states. We use the neural network value approximation estimator \code{NNES} of \citet{nguyen2025nnes} to fit the model on five thousand observations from five hundred individuals.

\begin{lstlisting}[style=econirlsession,caption={Fitting NNES on the Keane-Wolpin occupational choice panel. Reproduced by \code{code\_snippets/listing4\_nnes\_example.py}.},label={lst:nnes}]
>>> from econirl.datasets import load_keane_wolpin
>>> from econirl.estimation import NNES
>>> panel = load_keane_wolpin(as_panel=True)
>>> result = NNES(hidden_dim=64, num_layers=2, v_epochs=200,
...     outer_max_iter=100, n_outer_iterations=3).estimate(panel=panel)
>>> print(result.summary())
Log-Likelihood: -6698.66    Time: 43.5s (CPU) / 9.8s (A100 GPU)
\end{lstlisting}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{fig4_keane_wolpin_policy}
\caption{Modal occupational choice at age twenty over the schooling-by-white-collar-experience plane under the fitted NNES model. Reproduced by \code{code\_snippets/fig4\_keane\_wolpin\_policy.py} with seed 42 on NVIDIA A100 GPU, \pkg{econirl} 0.0.4.}
\label{fig:kw_policy}
\end{figure}
...
\begin{tabular}{l
                ...
                S[table-format=1.4]}
\toprule
\textbf{Estimator} & {$\theta_1$} & {$RC$} & {log-lik.} & {time (s)} & {cos.\ sim.} \\
\midrule
...
{\itshape $f$-IRL} & {-0.000000} & {0.0000} & -13862.95 & 133.8 & 0.0609 \\
\addlinespace[2pt]
BC          & {\textemdash} & {\textemdash} &  -4238.07 &   0.0 & {\textemdash} \\
\bottomrule
\end{tabular}
\caption{Cross-estimator benchmark on the Rust bus panel with ninety mileage states, two actions, and a discount factor of 0.9999. Columns report the operating cost coefficient $\theta_1$, the replacement cost $RC$, the maximized log-likelihood, the wall-clock time on a single central processing unit thread, and the cosine similarity between the fitted parameter vector and the ground truth. Behavioral cloning does not estimate structural parameters. Reproduced by \code{examples/rust-bus-engine/benchmark\_all\_estimators.py} with seed 42 on Apple M2 Pro CPU, \pkg{econirl} 0.0.4, \pkg{JAX} 0.4.30.}
\label{tab:bench}
\end{table}
