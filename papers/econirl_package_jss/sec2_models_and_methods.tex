...
\begin{algorithm}[H]
\SetAlgoLined
\Repeat{$\theta$ converges}{
  inner solve $V \leftarrow T_{\theta_k} V$ by SA, switching to Newton-Kantorovich \citep{iskhakov2016}\;
  evaluate $\ell(\theta_k)$ and per-observation scores via JAX\;
  update $\theta_{k+1}$ by BHHH or L-BFGS-B\;
}
...
\begin{algorithm}[H]
\SetAlgoLined
form augmented Lagrangian on $V - T_\theta V = 0$ with multipliers $\lambda(s)$\;
solve with SciPy SLSQP using JAX-computed Jacobians for both $\ell$ and the constraint\;
recover the structural Hessian by reduced-space projection on active constraints\;
...
\begin{algorithm}[H]
\SetAlgoLined
initialize $\hat\pi^{(0)}$ from frequency counts on the panel\;
\Repeat{$\theta$ converges}{
  build $\Psi^{(k)}(s, a) = \sigma \log \hat\pi^{(k)}(a \mid s) + \sigma \gamma$, back out $V$\;
  maximize $\ell$ in $\theta$ with $V$ fixed at the inversion\;
  update $\hat\pi^{(k+1)}$ from $\theta_{k+1}$ via equation~\eqref{eq:pi}\;
}
...
\begin{algorithm}[H]
\SetAlgoLined
warm-start $\xi$ from a least-squares fit of equation~\eqref{eq:V} on a coarse grid\;
...
\begin{algorithm}[H]
\SetAlgoLined
warm-start $V_\eta$ on a sub-grid of states by value iteration\;
...
\For{$k = 1, \dots, K$}{fit $\hat\pi^{(-k)}$ on the complement of fold $k$\;}
solve $\sum_{i} \psi(s_{it}, a_{it}; \theta, \hat\pi^{(-k(i))}) = 0$ across folds\;
estimate $\hat H$ from within-fold score outer products\;
...
\Repeat{$\|\nabla\| < \epsilon$ and $\|\mu_{\pi_{\theta_k}} - \hat\mu\|_\infty < \delta$}{
  soft value iteration at $\theta_k$ to obtain $\pi_{\theta_k}$, occupancy by direct solve \citep{gleave2022imitation}\;
  step $\theta_{k+1}$ by L-BFGS-B on $\hat \E[\phi] - \E_{\pi_{\theta_k}}[\phi]$\;
}
...
\begin{algorithm}[H]
\SetAlgoLined
build $\hat\mu$ from frequency counts on the panel\;
parametrize $Q$ tabularly or with a neural head\;
...
\Repeat{convergence}{
  ...
  step $\theta$ on the discriminator log-likelihood in equation~\eqref{eq:airl}\;
  step $\omega$ on $\log D_\theta - \log(1 - D_\theta)$ via PPO\;
}
...
\SetAlgoLined
\Repeat{$\theta$ converges}{
  rollout $\pi_\theta$ in the simulator and estimate $\mu_{\pi_\theta}$ from frequency counts\;
