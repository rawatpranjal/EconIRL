...
This section sets up a unified notation for dynamic discrete choice and inverse reinforcement learning, then walks through the twelve estimators that \pkg{econirl} implements. Each estimator subsection gives the objective, a compact pseudocode loop, and a short paragraph naming the variants exposed in the API. Proofs and convergence arguments are left to the cited papers.
...
\SetAlgoLined
initialize $\theta_0$\;
\Repeat{$\|\theta_{k+1} - \theta_k\| < \varepsilon$}{
  $V_k \leftarrow$ fixed point of $V = T_{\theta_k} V$ by SA-to-NK polyalgorithm \citep{iskhakov2016}\;
  $g_{it} \leftarrow \nabla_\theta \log \pi(a_{it} \mid s_{it}; \theta_k)$ via JAX implicit differentiation through $V_k$\;
  $\theta_{k+1} \leftarrow \theta_k + (\sum_{i,t} g_{it} g_{it}^\top)^{-1} \sum_{i,t} g_{it}$ \quad (BHHH step)\;
}
\Return $\hat\theta = \theta_{k+1}$, $\hat H = \sum_{i,t} g_{it} g_{it}^\top$\;
\caption{NFXP outer loop}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
assemble Lagrangian $\mathcal{L}(\theta, V, \lambda) = \ell(\theta, V) - \lambda^\top (V - T_\theta V)$\;
$(\hat\theta, \hat V, \hat\lambda) \leftarrow$ stationary point of $\nabla \mathcal{L} = 0$ via SciPy SLSQP\;
\quad with sparse Jacobians of $\ell$ and the constraint computed by JAX\;
$\hat H \leftarrow$ reduced-space projection of $\nabla^2_{\theta\theta} \mathcal{L}$ onto active constraints\;
\Return $\hat\theta$, $\hat H$\;
\caption{MPEC sequential quadratic programming}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
$\hat\pi^{(0)}(a \mid s) \leftarrow N^{-1}_s \sum_{i,t} \mathbb{1}\{s_{it} = s, a_{it} = a\}$ \quad (empirical CCP)\;
\Repeat{$\|\theta^{(k+1)} - \theta^{(k)}\| < \varepsilon$}{
  $V^{(k)}(s) \leftarrow \sigma \log \hat\pi^{(k)}(a \mid s) + \sigma \gamma + \beta \sum_{s'} P(s' \mid s, a) V^{(k)}(s')$ \quad (Hotz-Miller inversion)\;
  $\theta^{(k+1)} \leftarrow \argmax_\theta \sum_{i,t} \log \pi_\theta(a_{it} \mid s_{it} ; V^{(k)})$ \quad (one Newton step)\;
  $\hat\pi^{(k+1)} \leftarrow \pi_{\theta^{(k+1)}}$ via equation~\eqref{eq:pi}\;
}
\caption{Nested pseudo-likelihood for CCP}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
$\xi_0 \leftarrow \argmin_\xi \sum_{s \in \text{grid}} (V_\xi(s) - T_{\theta_0} V_\xi(s))^2$ \quad (OLS warm-start)\;
$(\hat\theta, \hat\xi) \leftarrow \argmax_{\theta, \xi} \ell(\theta, \xi) - \kappa \sum_s (V_\xi(s) - T_\theta V_\xi(s))^2$ via L-BFGS-B\;
$\hat H \leftarrow \nabla^2_{\theta\theta} \ell(\theta, \hat\xi(\theta))$ at $\theta = \hat\theta$\;
\caption{Sieve joint maximum likelihood}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
warm-start $V_\eta$ on a sub-grid by least-squares regression on $V^\ast_{\theta_0}$\;
\For{epoch $k$ until validation loss plateaus}{
  sample minibatch $\mathcal{B}$; $(\theta, \eta) \leftarrow \mathrm{Adam}$ on equation~\eqref{eq:nnes} restricted to $\mathcal{B}$\;
}
profile $\hat H \leftarrow \nabla^2_{\theta\theta} \ell(\theta, \hat\eta(\theta))$ at $\hat\theta$ via JAX\;
\caption{Neural value approximation}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
partition individuals $\{1, \dots, N\}$ into $K$ disjoint folds $\mathcal{I}_1, \dots, \mathcal{I}_K$ at random\;
\For{$k = 1, \dots, K$}{
  $\hat\pi^{(-k)} \leftarrow \argmin_\pi \sum_{i \notin \mathcal{I}_k, t} \mathrm{CE}(\pi(s_{it}), a_{it})$ \quad (softmax network on complement)\;
}
$\hat\theta \leftarrow$ root of $\sum_{k=1}^K \sum_{i \in \mathcal{I}_k, t} \psi(s_{it}, a_{it}; \theta, \hat\pi^{(-k)}) = 0$ via Newton\;
$\hat H \leftarrow K^{-1} \sum_k \big( \sum_{i \in \mathcal{I}_k, t} \psi \psi^\top \big)$\;
...
\begin{algorithm}[H]
\SetAlgoLined
$\hat \E[\phi] \leftarrow n^{-1} \sum_{i,t} \phi(s_{it}, a_{it})$\;
initialize $\theta_0$\;
\Repeat{$\|g_k\| < \varepsilon$ \textbf{and} $\|\mu_{\pi_{\theta_k}} - \hat\mu\|_\infty < \delta$}{
  $\pi_{\theta_k} \leftarrow$ soft value iteration at reward $r_{\theta_k}$\;
  $\mu_{\pi_{\theta_k}} \leftarrow (I - \beta P_{\pi_{\theta_k}})^{-1} \rho_0$ \quad (direct linear solve, \citealp{gleave2022imitation})\;
  $g_k \leftarrow \hat \E[\phi] - \sum_{s,a} \mu_{\pi_{\theta_k}}(s, a) \phi(s, a)$\;
  $\theta_{k+1} \leftarrow$ L-BFGS-B step in direction $g_k$\;
}
...
\begin{algorithm}[H]
\SetAlgoLined
$\hat\mu(s, a) \leftarrow n^{-1} \sum_{i,t} \mathbb{1}\{s_{it} = s, a_{it} = a\}$\;
initialize $Q$ tabularly or with a neural head; $Q \leftarrow \mathrm{Adam}$ on equation~\eqref{eq:iql} until plateau\;
$\hat r(s, a) \leftarrow \hat Q(s, a) - \beta \sum_{s'} P(s' \mid s, a) \hat V(s')$ \quad (inverse Bellman)\;
\caption{Inverse soft Q-learning}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
initialize $Q_\eta$, $V_\zeta$\;
\For{epoch $k$ until validation loss plateaus}{
  sample minibatch $\mathcal{B}$ of transitions $(s, a, s')$; $(\eta, \zeta) \leftarrow \mathrm{Adam}$ on equation~\eqref{eq:gladius}\;
}
$\hat r(s, a) \leftarrow$ average of $Q_\eta(s, a) - \beta V_\zeta(s')$ over observed $s'$\;
\caption{GLADIUS dual-network training}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
initialize reward $r_\theta$, shaping $h_\theta$, generator policy $\pi_\omega$\;
\Repeat{discriminator and generator stabilize}{
  $\mathcal{B}_E \leftarrow$ batch from panel; $\mathcal{B}_\pi \leftarrow$ rollouts of $\pi_\omega$ in the simulator\;
  $\theta \leftarrow \mathrm{Adam}\!\left( \nabla_\theta \big[ \sum_{\mathcal{B}_E} \log D_\theta + \sum_{\mathcal{B}_\pi} \log(1 - D_\theta) \big] \right)$\;
  $\omega \leftarrow \mathrm{PPO}$ step on rollouts $\mathcal{B}_\pi$ with reward $\tilde r = \log D_\theta - \log(1 - D_\theta)$\;
}
\Return $\hat r_\theta$ \quad (disentangled by the parameterization in equation~\eqref{eq:airl})\;
...
\begin{algorithm}[H]
\SetAlgoLined
initialize reward $r_\theta$; choose $f$ from $\{\mathrm{KL}, \mathrm{revKL}, \mathrm{JS}, \chi^2\}$\;
$\hat\mu \leftarrow$ empirical state-action occupancy from the panel\;
\Repeat{$\|\nabla_\theta\| < \varepsilon$}{
  $\pi_\theta \leftarrow$ soft-RL policy under reward $r_\theta$; rollout in simulator\;
  $\mu_{\pi_\theta} \leftarrow$ frequency counts on rollouts\;
  $\theta \leftarrow \theta - \alpha \nabla_\theta D_f^\ast(\mu_{\pi_\theta} \,\|\, \hat\mu)$ \quad (Fenchel dual gradient)\;
}
\caption{$f$-divergence IRL}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
$X \leftarrow [\phi(s_{it}, \cdot)]_{(i,t)}$, $y \leftarrow [a_{it}]_{(i,t)}$\;
$\hat\theta \leftarrow \argmin_\theta -\sum_{i,t} \log \mathrm{softmax}(X_{it} \theta)_{y_{it}}$ via L-BFGS-B\;
$\hat H \leftarrow \nabla^2_\theta$ of the cross-entropy at $\hat\theta$\;
...
Table~\ref{tab:taxonomy} reads as a decision aid. A researcher with a known transition matrix and a small state space picks from the first three rows. The same researcher with a large state space picks from rows four through six. A researcher who wants to recover the reward nonparametrically picks from rows seven through eleven, with the choice driven by whether transitions are known and whether the reward should transfer across dynamics. Behavioral cloning in the last row is always available as a sanity check.
...
\bottomrule
\end{tabular}
\caption{The twelve production estimators along their structural assumptions. Columns mirror the \code{ProblemCapabilities} dataclass in \code{econirl.estimation.categories}, regenerated whenever a new estimator is added.}
\label{tab:taxonomy}
\end{table}
