...
This section sets up a unified notation for dynamic discrete choice and inverse reinforcement learning, then walks through the twelve estimators that \pkg{econirl} implements. Each estimator subsection has the same shape. We give the objective the estimator targets, a compact pseudocode block, and a short paragraph naming the variants exposed in the API. Proofs are left to the cited papers.
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, $P$, $\phi$, initial $(\theta_0, V_0)$}
\KwOut{$(\hat\theta, \hat V)$, Lagrangian Hessian}
assemble sparse Jacobian of the Bellman residual via \pkg{JAX}\;
form the augmented Lagrangian with multipliers $\lambda(s)$ on $V(s) - (T_\theta V)(s) = 0$\;
solve with SciPy SLSQP using analytic gradients for both $\ell$ and the constraint\;
recover the structural Hessian by reduced-space projection on the active constraints\;
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, $P$, $\phi$, initial $\hat\pi^{(0)}$ from frequency counts}
\KwOut{$\hat\theta$, $\hat\pi^{(\infty)}$}
\For{$k = 0, 1, \dots$ until $\theta$ converges}{
  build $\Psi(s, a) = \sigma \log \hat\pi^{(k)}(a \mid s) + \sigma \gamma$\;
  back out $V$ from $\Psi$ using the choice-specific value identity\;
  maximize $\ell$ in $\theta$ with $V$ held fixed at the inversion\;
  update $\hat\pi^{(k+1)}$ from the new $\theta_{k+1}$ via equation~\eqref{eq:pi}\;
}
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, $P$, $\phi$, basis $b_{1:K}$, penalty $\kappa$}
\KwOut{$\hat\theta$, $\hat\xi$}
initialize $\xi$ from a least-squares fit of equation~\eqref{eq:V} on a coarse grid\;
jointly maximize the penalized log-likelihood in $(\theta, \xi)$ with L-BFGS-B\;
recompute the Hessian of the profile likelihood in $\theta$ with $\xi$ at its optimum\;
\caption{Sieve joint maximum likelihood}
\end{algorithm}

\paragraph{Versions.} The \code{SEES} class exposes polynomial and B-spline bases, with the order $K$ chosen by the user or selected by leave-one-out cross-validation. The penalty weight $\kappa$ defaults to one and can be tuned. Setting $\kappa$ to infinity recovers exact value iteration in the basis.
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, $P$, $\phi$, network $V_\eta$, penalty $\kappa$}
\KwOut{$\hat\theta$, $\hat\eta$}
initialize $\eta$ by fitting $V_\eta$ to value iteration on a sub-grid of states\;
\For{epoch $k = 0, 1, \dots$}{
  sample minibatches of state-action pairs from the panel\;
  step $(\theta, \eta)$ jointly with Adam on the penalized log-likelihood\;
}
profile the Hessian in $\theta$ via JAX automatic differentiation\;
\caption{Neural value approximation}
\end{algorithm}

\paragraph{Versions.} The \code{NNES} class exposes the network width and depth, the optimizer (Adam by default, SGD optional), and the Bellman penalty weight. Standard errors are available because the Hessian is taken in $\theta$ at the profiled $\hat\eta$.
...
\paragraph{Versions.} The \code{TDCCP} class exposes the fold count $K$ (default five), the split rule (individual or row, individual is the default and the only one with valid cross-fitting), and the neural CCP head architecture. Inference is asymptotically valid under cross-fitting, which is the practical edge over NFXP on high-dimensional state spaces.
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, $P$, $\phi$ (used only post hoc), penalty $\kappa$}
\KwOut{$\hat Q$, recovered reward $\hat r$}
build $\hat\mu$ from the panel by frequency counts\;
parametrize $Q$ tabularly (small $|\mathcal{S}| |\mathcal{A}|$) or by a neural head\;
minimize equation~\eqref{eq:iql} jointly in $Q$ with Adam\;
recover $\hat r(s, a) = \hat Q(s, a) - \beta \E_{s'} \hat V(s')$\;
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, $\phi$ (used only post hoc), networks $Q_\eta, V_\zeta$, penalty $\kappa$}
\KwOut{$\hat\eta, \hat\zeta$, recovered reward $\hat r$}
sample minibatches of $(s_{it}, a_{it}, s_{it+1})$ from the panel\;
step $(\eta, \zeta)$ with Adam on equation~\eqref{eq:gladius}\;
recover $\hat r(s, a) = Q_\eta(s, a) - \beta V_\zeta(s')$ averaged over observed $s'$\;
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, environment simulator, reward net $r_\theta$, $f$-divergence}
\KwOut{$\hat\theta$}
\For{epoch $k = 0, 1, \dots$}{
  sample policy rollouts under $\pi_\theta$\;
  estimate $\mu_{\pi_\theta}$ by frequency counts on the rollouts\;
  step $\theta$ on the Fenchel dual of $D_f(\mu_{\pi_\theta} \,\|\, \hat\mu)$\;
}
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{panel, features $\phi$}
\KwOut{$\hat\theta$}
form the design matrix from $\phi(s_{it}, a_{it})$ across all $(i, t)$\;
fit a multinomial logit by L-BFGS-B on the cross-entropy\;
