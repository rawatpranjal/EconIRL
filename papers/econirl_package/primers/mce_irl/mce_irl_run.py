...
def render_results_tex(records: list[dict[str, Any]]) -> str:
    ...
    add(
        "The validation target is the tabular maximum causal entropy estimator "
        "with known transitions and known action-dependent reward features. "
        "Hard gates are enforced on feature matching, occupancy moments, "
        "normalized reward/value/Q recovery, policy distance, and Type A/B/C "
        "counterfactual regret."
    )
    add("")
    add(r"\subsection{Validation Design Context}")
    add(validation_context(list(records_by_id)))
    add("")
    ...
    add(
        "Raw parameter cosine is intentionally not a validation gate for "
        "MCE-IRL because feature matching, occupancy moments, reward/value/Q "
        "recovery, policy distance, and counterfactual regret are the "
        "decision-critical validation objects."
    )
    add("")
...
def compact_payload(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    all_gates_pass = all(
        ...
        else getattr(gate, "passed", False)
