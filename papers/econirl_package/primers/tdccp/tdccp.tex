...
CCP estimation \citep{hotzMiller1993} avoids the inner Bellman loop of NFXP by inverting observed choice probabilities into value differences.
...
\citet{adusumilliEckardt2025} eliminate the transition density entirely.
...
NFXP \citep{rust1987} and CCP require known transitions.
NNES \citep{nguyen2025} and SEES \citep{luoSang2024} require transitions for the Bellman target.
...
The semi-gradient fixed-point equation~\eqref{eq:semigradient} is the structural econometrics analog of the \citet{tsitsiklisVanRoy1997} linear TD method with function approximation.
...
The CCP decomposition of \citet{hotzMiller1993} writes the choice-specific value as
\begin{equation}
\label{eq:ccp_decomp}
...
Cross-fitting \citep{chernozhukov2018} splits the data into two folds, estimating $h,g$ on one fold and $\theta$ on the other. This eliminates the Donsker condition that would otherwise restrict the complexity of the first-stage estimator.
...
\begin{algorithm}[H]
\DontPrintSemicolon
\caption{TD-CCP with Semi-Gradient \citep{adusumilliEckardt2025}}
\label{alg:tdccp}
...
Flow utility is $u(1, z;\theta) = \theta_v z + \theta_{FC}$ and $u(0, z;\theta) = 0$, where $\theta_v = 5.0$ and $\theta_{FC} = -1.0$.
The state is discretized to \tdNstates\ bins approximating the continuous process.
The dataset has \tdNobs\ observations from 3{,}000 individuals over 5 periods with $\beta = \tdBeta$.

CCP must discretize the state into $K$ coarse bins and estimate the transition matrix $\hat P(z' \mid z, a)$ by frequency counting.
With $K = 5$ bins, each bin spans a productivity range of $0.2$, averaging over a utility variation of $\theta_v \times 0.2 = 1.0$, which is larger than both structural parameters combined.
The coarse features bias the parameter estimates away from the truth.
The natural fix is to increase $K$: at $K = 100$ bins (the same resolution as the fine grid, so no coarsening at all), the AR(1) noise spreads each source state across roughly 17 destination bins.
With \tdNobs\ observations and two actions, each source state receives about 75 observations; spread across 17 active transition cells, that yields roughly 4 observations per cell.
The frequency estimate $\hat P$ is nearly noise, and the NPL propagates this noise directly into the parameter estimates.
There is no safe choice of $K$: coarser bins introduce feature bias, finer bins introduce transition estimation variance.
This is the Goldilocks trap for discretized CCP.
TD-CCP uses degree-8 polynomial basis functions over $z_{\mathrm{norm}} \in [0,1]$ and never forms $\hat P$ at all.

Table~\ref{tab:tdccp_results} shows the outcome.
TD-CCP achieves RMSE of \tdRMSE, with robust standard errors in parentheses.
CCP with $K = 5$ bins reaches RMSE of \ccpFiveRMSE\ (feature bias dominates), \ccpFiveWin\ times worse.
CCP with $K = 100$ bins reaches RMSE of \ccpHundredRMSE\ (transition noise dominates), \ccpHundredWin\ times worse.
Finer discretization makes things worse, not better, once the transition matrix is too sparse to estimate reliably.
Behavioral cloning reaches \bcAcc\ policy accuracy but recovers no structural parameters.

A mini Monte Carlo (\mcNreps\ replications, \mcNobs\ observations each) reveals that the robust standard errors understate uncertainty: 95\% confidence interval coverage is \mcCovV\ for $\theta_v$ and \mcCovFC\ for $\theta_{FC}$, against a nominal 95\% target. The SE formula conditions on $\hat h$ and $\hat g$ as if they were known, but they are estimated from the same data. The locally robust correction in Equation~\eqref{eq:robust_moment} accounts for this asymptotically, but computing the full Jacobian requires re-estimating $h,g$ at each $\theta$ perturbation. The decisive RMSE win stands; the coverage issue affects inference, not point estimation. Cross-fitting is disabled here because the states are discretized to \tdNstates\ bins, so CCP estimates converge at the parametric rate. For genuinely continuous states, cross-fitting should be enabled per the paper's recommendation.
